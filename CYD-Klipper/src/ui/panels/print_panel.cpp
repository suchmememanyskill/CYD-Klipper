...
static void BtnPrintFile(lv_event_t * e){
    lv_obj_t * panel = (lv_obj_t*)lv_event_get_user_data(e);
    lv_obj_del(panel);

    char* buff = (char*)malloc(128 + (strlen(selected_file->name) * 3));
    sprintf(buff, "http://%s:%d/printer/print/start?filename=", globalConfig.klipperHost, globalConfig.klipperPort);
...
static void BtnPrintFileVerify(lv_event_t * e){
    ...
    lv_obj_set_size(btn, CYD_SCREEN_MIN_BUTTON_WIDTH_PX * button_size_mult, CYD_SCREEN_MIN_BUTTON_HEIGHT_PX * button_size_mult);
    lv_obj_add_event_cb(btn, BtnPrintFile, LV_EVENT_CLICKED, panel);
...
void PrintPanelInit(lv_obj_t* panel){
    ...
    while (files != NULL && files->name != NULL && count <= 20){
        ...
        lv_obj_add_event_cb(btn, BtnPrintFileVerify, LV_EVENT_CLICKED, (void*)files);
