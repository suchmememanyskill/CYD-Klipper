...
void MacrosPanelAddMacrosToPanel(lv_obj_t * root_panel, MacrosQuery query){
...
void MacrosPanelAddPowerDevicesToPanel(lv_obj_t * root_panel, PowerQuery query){
    for (int i = 0; i < query.count; i++){
...
void MacrosPanelInit(lv_obj_t* panel) {
    ...
    MacrosQuery query = macrosQuery();
    PowerQuery power = powerDevicesQuery();
