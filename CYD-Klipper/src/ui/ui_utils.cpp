...
void LayoutFlexColumn(lv_obj_t* obj, lv_flex_align_t align, lv_coord_t pad_column, lv_coord_t pad_row){
...
void LayoutFlexRow(lv_obj_t* obj, lv_flex_align_t align, lv_coord_t pad_column, lv_coord_t pad_row){
    lv_obj_set_layout(obj, LV_LAYOUT_FLEX);
...
void CreateFullscreenButtonMatrixPopup(lv_obj_t * root, lv_event_cb_t title, LvButtonColumn_t* columns, int column_count){
    ...
    lv_obj_t * top_menu_row = CreateEmptyPanel(panel);
    ...
    lv_obj_add_event_cb(btn, DestroyEventUserData, LV_EVENT_CLICKED, panel);
    ...
    lv_obj_t * rows = CreateEmptyPanel(panel);
    lv_obj_set_size(rows, FULL_PANEL_INNER_WIDTH, column_height);
    lv_obj_align(rows, LV_ALIGN_BOTTOM_LEFT, 0, 0);
    LayoutFlexRow(rows, LV_FLEX_ALIGN_SPACE_BETWEEN, CYD_SCREEN_GAP_PX, CYD_SCREEN_GAP_PX);

    for (int i = 0; i < column_count; i++){
        lv_obj_t * column = CreateEmptyPanel(rows);
        lv_obj_clear_flag(column, LV_OBJ_FLAG_SCROLLABLE);
        lv_obj_set_size(column, column_width, column_height);
        LayoutFlexColumn(column, LV_FLEX_ALIGN_CENTER, CYD_SCREEN_GAP_PX, CYD_SCREEN_GAP_PX);
