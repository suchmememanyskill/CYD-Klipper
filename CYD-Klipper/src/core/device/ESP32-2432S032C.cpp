#ifdef CYD_SCREEN_DRIVER_ESP32_2432S032C
...
void screen_lv_touchRead(lv_indev_drv_t *indev_driver, lv_indev_data_t *data)
{
    ...
    else
    {
        ...
        for (int i = 0; i < tp.touches; i++)
        {
            ...
            {
                magicY = TOUCH_HEIGHT- (TOUCH_WIDTH - tp.points[i].x);
            ...
            else
            {
                ...
                magicY = TOUCH_WIDTH - tp.points[i].x;
                magicX = TOUCH_WIDTH - (TOUCH_HEIGHT - tp.points[i].y);
            }
...
void screen_setup()
{
    ...
    tft.setRotation(global_config.rotateScreen ? 7 : 5);
...
#endif // CYD_SCREEN_DRIVER_ESP32_2432S032C